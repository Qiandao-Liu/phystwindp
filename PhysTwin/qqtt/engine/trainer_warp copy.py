...
class InvPhyTrainerWarp:
    ...
    def replay_all_trajectories(trainer):
        import glob
        from pathlib import Path

        mpc_dir = Path("~/workspace/PhysTwin/temp_mpc_output/rope_double_hand").expanduser()
        save_dir = Path("~/workspace/PhysTwin/mpc_replay_outputs").expanduser()
        save_dir.mkdir(parents=True, exist_ok=True)

        all_pkl = sorted(mpc_dir.glob("*.pkl"))

        for pkl_path in tqdm(all_pkl):
            with open(pkl_path, "rb") as f:
                data = pickle.load(f)
            ctrl_traj = data["ctrl_traj"]

            trainer.simulator.set_init_state(
                trainer.simulator.wp_init_vertices, trainer.simulator.wp_init_velocities
            )

            for i, ctrl_points in enumerate(ctrl_traj):
                ctrl_tensor = torch.tensor(ctrl_points, dtype=torch.float32, device=cfg.device)

                trainer.simulator.set_controller_interactive(
                    trainer.simulator.controller_points[0], ctrl_tensor
                )
                ...
                x = wp.to_torch(trainer.simulator.wp_states[-1].wp_x, requires_grad=False)[:trainer.num_all_points]
                gs_xyz = x.detach().cpu().numpy()
                ctrl_xyz = ctrl_tensor.detach().cpu().numpy()

                idx = int(pkl_path.stem.split("_")[-1]) * 1000 + i  # 避免冲突，每个 rollout 分段
            ...
            print(f"✅ Replayed {pkl_path.name} into {len(ctrl_traj)} frames.")
    ...
    def set_init_state_from_numpy(self, gs_np, ctrl_np):
        ...
        self.simulator.set_init_state(
            gs_tensor,
            torch.zeros_like(gs_tensor),   # 假设初始速度为0
            pure_inference=True
        )
    ...
    def on_press(self, key):
        try:
            ...
            if key_char == "p":
                now = time.time()
                if now - self.last_p_toggle > 0.5:  # 500ms 去抖动
    ...
    def interactive_playground(
        ...
        while True:
            ...
            if self.recording and np.linalg.norm(target_change) > 0:
